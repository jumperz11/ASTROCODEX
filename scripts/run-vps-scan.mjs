...
const researchPrompt =
  "Check @astronomer_zero's newest relevant public X posts and connected threads. Compare them with the latest accepted Astro Intelligence forecast. Apply the archived playbook and treat the three scenarios as a next-move prediction engine: each scenario must predict Astro's next observable behavior, be ordered highest probability first, and use his best-supported current position, relevant Astro Codex behavior, distance to verified levels, and the supplied live market snapshot. Save a forecast when new direct evidence materially changes the read, OR when verified market movement changes the leading predicted behavior, crosses a relevant Astro/model level, or shifts a scenario weight by at least 10 points. A model-only update may change scenarios and thesis while Astro is silent, but it must keep the confirmed signal at wait unless fresh direct evidence supports long, short, take_profit, exit, or conflict. Never turn archive similarity or market movement alone into a confirmed trade. Preserve exact direct status URLs and the separation between Astro evidence and inference. If nothing material changed, do not save.";
